...
# This allows declaration	use VirulenceFinder ':all';
...
Example of use with the *database* folder located in the current directory
    ...
    VirulenceFinder.pm -i test.fsa -s virulence_ecoli -k 95.00

Example of use with the *database* loacted in another directoriy and a specified output folder

    VirulenceFinder.pm -d path/to/database -i test.fsa -o OUTFOLDER -s virulence_ecoli -k 95.00  
